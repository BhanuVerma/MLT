...
def find_optimal_allocations(prices):
    ...
    no_of_stocks = (len(prices.columns))
    no_of_stocks_float = float(no_of_stocks)
    allocs = [1.0/no_of_stocks_float for x in range(no_of_stocks)]
    ranges = [(0, 1) for k in range(no_of_stocks)]
    constraints = ({'type': 'eq', 'fun': lambda x:  1 - np.sum(x)})
    min_results = spo.minimize(find_sharpe_ratio, allocs, args=(prices,), method = 'SLSQP', bounds = ranges,constraints=constraints, options={'disp':True})
    return min_results.x
